...
cls
color 2f
echo ПЯТЬ НОЧЕЙ СО СТРАНАМ (v2.1)
echo Ваша задача - любым способом спастись от Украины.
echo Если вы столкнётесь с Украиной - конец игры.
echo.
echo 1 - Начать игру     2 - Выйти     3 - Бонусный уровень
echo.
echo.
echo.
choice /C 123 /M ""
if %errorlevel% == 2 goto cd programs && closeprogram
...
cls
echo *рассвет*
echo Поздравляем! Ночь пройдена! А теперь усложнения... Приготовьтесь к ночи 2...
...
echo.
if %prc% GTR 99 set prc=0&& goto n33
SET /A RND=%RANDOM% %% 4
ping 127.0.0.1 -n %RND% > nul
SET /A RNDD=%RANDOM% %% 20
if %RNDD% == 8 echo Тсс, где-то рядом бегает Украина... Подождите пару секунд...&& ping 127.0.0.1 -n 6 > nul&& echo Вроде убежала, можно открывать двери...&& ping 127.0.0.1 -n 2 > nul
...
cls
echo *рассвет*
echo Поздравляем! Ночь пройдена! Так держать! До третьей ночи не с первой попытки дойдёшь! Приготовьтесь к ночи 3...
...
echo.
if %prc% GTR 149 set prc=0&& goto n44
SET /A RND=%RANDOM% %% 4
ping 127.0.0.1 -n %RND% > nul
SET /A RNDD=%RANDOM% %% 20
if %RNDD% == 8 echo Тсс, где-то рядом бегает Украина... Подождите пару секунд...&& ping 127.0.0.1 -n 6 > nul&& echo Вроде убежала, можно открывать двери...&& ping 127.0.0.1 -n 2 > nul
...
cls
echo *рассвет*
echo Поздравляем! Дойти до 4 ночи - уже ДОСТИЖЕНИЕ! Ноооо....приготовьтесь к ночи 4...... Это пипец....
...
echo.    Принято стран: %prc%/150
echo.
if %prc% GTR 149 set prc=0&& goto n55
SET /A RND=%RANDOM% %% 4
ping 127.0.0.1 -n %RND% > nul
SET /A RNDD=%RANDOM% %% 20
if %RNDD% == 8 echo Тсс, где-то рядом бегает Украина... Подождите пару секунд...&& ping 127.0.0.1 -n 6 > nul&& echo Вроде убежала, можно открывать двери...&& ping 127.0.0.1 -n 2 > nul
...
cls
echo *рассвет*
echo Поздравляем!!! Ночь пройдена! Вы можете считать, ЧТО ВЫ ВЫИГРАЛИ... Но... есть ПЯТАЯ НОЧЬ...приготовьтесь к НЕЙ...... Это пи-пец, но зато ПОЛНЫЙ конец!
...
echo.    Страны, приходяшие с любой страной на букву У: Литва, Латвия, Эстония, Япония, США, Азербайджан, Грузия, Германия, Финляндия
echo.    МИКРОНЕЗИЯ и ПОЛЬША отдаёт вас УКРАИНЕ.
echo.    Принято стран: %prc%/175
echo.
if %prc% GTR 174 set prc=0&& goto win
SET /A RND=%RANDOM% %% 2
ping 127.0.0.1 -n %RND% > nul
SET /A RNDD=%RANDOM% %% 20
if %RNDD% == 8 echo Тсс, где-то рядом бегает Украина... Подождите пару секунд...&& ping 127.0.0.1 -n 6 > nul&& echo Вроде убежала, можно открывать двери...&& ping 127.0.0.1 -n 2 > nul
...
cls
echo.
echo Бонусный уровень пройден! ПОЗДРАВЛЯЕМ!

echo Нажмите на любую клавишу, чтобы выйти в главное меню!
pause > nul
5nc.bat
...
if "%country%" == "Грузия" goto otdali
if "%country%" == "Германия" goto otdali
if "%country%" == "Финляндия" goto otdali
if "%country%" == "Австрия" goto otdali
if "%country%" == "Великобритания" goto otdali
...
set "country_list[187]=Ватикан" 
set "country_list[188]=Палестина" 
set "country_list[189]=Латвия" 
set "country_list[190]=Латвия" 
set "country_list[191]=Латвия" 
set "country_list[192]=Латвия" 
...
echo.
color 2
echo Вы можете продолжить игру. Но вы потеряли патрон...
ping 127.0.0.1 -n 2 > nul
goto %n%
) 
ping 127.0.0.1 -n 3 > nul
...
echo cd programs && closeprogram >> remove5nc.bat
cd ..
cd ..
